...
const VisitHistoryList = ({ dataForm }) => {
  ...
  const {
    ...
    "visitbyuser",
    `/allVisit/report/${dataForm.fromDate}/${dataForm.tillDate}`
  );
  ...
  return (
    <>
      <div className="flex justify-end items-center">
        <PdfButton
          path={`/opsPdf/report/${dataForm.fromDate}/${dataForm.tillDate}`}
        />
          ...
          data.map((item, index) => (
            ...
            >
              ...
              <div className="flex justify-end space-x-1">
                <TaskButton
                  path={`/ops/visit/report/preview/${item.allVisitId}`}
                  btnColor="btn-gray"
                />
              </div>
            </div>
