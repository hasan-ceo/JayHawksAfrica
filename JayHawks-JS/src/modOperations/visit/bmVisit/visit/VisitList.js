...
const VisitList = ({ dataForm }) => {
  const {
    ...
    "visitbyuser",
    `/bmvisit/listbyuser/${dataForm.fromDate}/${dataForm.tillDate}`
  );
  ...
  return (
    <>
      <PrintHeader title="" fileName="myvisitlist.csv" data={data} />
          ...
          list.data.map((item) => (
            ...
            >
              ...
              <div className="flex justify-end space-x-2">
                <TaskButton
                  path={`/ops/bm/visit/preview/${item.bmVisitId}`}
                  btnColor={item.isSubmit === 0 ? "btn-umojayellow" : "btn-gray"}
                />
                {item.isSubmit === 0 && (
                  <>
                    <EditButton path={`/ops/bm/visit/edit/${item.bmVisitId}`} />
                    <DeleteButton
                      action={refetch}
                      path={`/bmvisit/delete/${item.bmVisitId}`}
                    />
                  </>
                )}
              </div>
