...
const VisitList = ({ dataForm }) => {
  const {
    ...
    "visitbyuser",
    `/amvisit/listbyuser/${dataForm.fromDate}/${dataForm.tillDate}`
  );
          ...
          list.data.map((item) => (
            ...
            >
              ...
              <div className="flex justify-end space-x-2">
                <TaskButton
                  path={`/ops/am/visit/preview/${item.amVisitId}`}
                  btnColor={
                ...
                {item.isSubmit === 0 && (
                  <>
                    <EditButton path={`/ops/am/visit/edit/${item.amVisitId}`} />
                    <DeleteButton
                      action={refetch}
                      path={`/amvisit/delete/${item.amVisitId}`}
                    />
                  </>
                )}
              </div>
