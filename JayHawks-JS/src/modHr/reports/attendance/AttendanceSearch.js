import React, { useState } from "react";
import { useGetData } from "../../../hooks/dataApi";
import { HashLoading } from "../../../components/Loading";
import Error from "../../../components/Error";
import { ListHeader, ListCol } from "../../../components/ListColWithHeader";
import { format } from "date-fns";
import SearchHeader from "../../../components/SearchHeader";
import PrintHeader from "../../../components/PrintHeader";
import PdfButton from "../../../components/button/PdfButton";

const AttendanceSearch = ({ dataForm }) => {
  const [query, setQuery] = useState("");

  const {
    data: list,
    error,
    isLoading,
    isError,
  } = useGetData(
    "empattendance",
    `/empattendance/date/${dataForm.fromDate}/${dataForm.tillDate}`
  );
  if (isLoading) return <HashLoading />;

  if (isError) return <Error message={error?.message} />;

  const data = list.data
    .filter((item) => {
      if (query === "") {
        return item;
      } else if (
        item.employeePin.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
        item.employeeName.toLowerCase().indexOf(query.toLowerCase()) !== -1
      ) {
        return item;
      } else return null;
    })
    .map(
      ({
        empAttendanceId,
        branchName,
        departmentName,
        employeePin,
        employeeName,
        designationName,
        workDate,
        attenStatus,
        particulars,
        manualEntry,
      }) => ({
        empAttendanceId,
        branchName,
        departmentName,
        employeePin,
        employeeName,
        designationName,
        workDate,
        attenStatus,
        particulars,
        manualEntry,
      })
    );

  return (
    <>
      <div className="flex justify-end items-center">
        <PdfButton
          path={`/HrPdfReport/attendance/${dataForm.fromDate}/${dataForm.tillDate}`}
        />

        <PrintHeader title="" fileName="attendance.csv" data={data} />
      </div>
      <SearchHeader placeholder="PIN / Name " action={setQuery} />

      <div className="list-wrapper">
        <div className="md:grid grid-cols-9 list-header">
          <ListHeader label="Branch" />
          <ListHeader label="Department" />
          <ListHeader label="PIN" />
          <ListHeader label="Employee Name" />
          <ListHeader label="Designation" />
          <ListHeader label="Date" />
          <ListHeader label="Status" />
          <ListHeader label="Particulars" />
          <ListHeader label="Manual" />
        </div>
        {data.length > 0 &&
          data.map((item) => (
            <div
              key={item.empAttendanceId}
              className="grid grid-cols-1 md:grid-cols-9 list-body"
            >
              <ListCol label="Branch:" value={item.branchName} />
              <ListCol label="Department: " value={item.departmentName} />
              <ListCol label="PIN : " value={item.employeePin} />
              <ListCol label="Employee Name: " value={item.employeeName} />
              <ListCol label="Designation: " value={item.designationName} />
              <ListCol
                label="Date: "
                value={format(new Date(item.workDate), "dd/MMM/yyyy")}
              />
              <ListCol label="Status: " value={item.attenStatus} />
              <ListCol label="Particulars: " value={item.particulars} />
              <ListCol
                label="Manual: "
                value={item.manualEntry ? "Yes" : "No"}
              />
              <div></div>
            </div>
          ))}

        <div className="list-footer">
          <div className="col-span-10"></div>
          <div className="flex justify-center">
            <span className="font-semibold">Total : {data.length}</span>
          </div>
        </div>
      </div>
    </>
  );
};

export default AttendanceSearch;
