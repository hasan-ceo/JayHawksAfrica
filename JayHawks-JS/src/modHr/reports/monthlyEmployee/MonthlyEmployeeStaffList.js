...
const MonthlyEmployeeStaffList = ({ dataForm }) => {
  ...
  const {
    ...
    "employees",
    `/hrreports/monthlyEmpolyee/${dataForm.fromDate}/${dataForm.tillDate}`
  );
  ...
  return (
    <div>
      <div className="flex justify-end items-center">
        <PdfButton
          path={`/HrPdfReport/monthlyEmpolyee/${dataForm.fromDate}/${dataForm.tillDate}`}
        />
        <PrintHeader
          // title="Monthly Employee List"
          fileName="monthlyEmpolyee.csv"
          data={data.map(
            ({
              salaryYearMonth,
              branchName,
      ...
      <div className="list-wrapper">
        ...
        {data.length > 0 &&
          data.map((item) => (
            ...
            >
              <ListCol label="Month of : " value={item.salaryYearMonth} />
              ...
              <ListCol
                ...
                value={
                  item.grossSalaryUsd === 0
                    ? item.grossSalary.toLocaleString("en-US") + " UGX"
...
export default MonthlyEmployeeStaffList;
