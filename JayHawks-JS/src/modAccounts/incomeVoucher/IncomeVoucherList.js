...
const IncomeVoucherList = () => {
  const {
    ...
    isError,
    refetch,
  } = useGetData("incomeVoucher", "/accountGl/incomeList");
  ...
  return (
    <div className="card w-full max-w-screen-xl">
      <div className="flex justify-between px-0 py-2 items-cente ">
        <h1 className="text-xl lg:text-2xl font-bold lg:text-semibold text-gray-600 capitalize ">
          Income Voucher List
        </h1>
        <div className="flex justify-end space-x-2 ">
          <TextButton path="/ac/IncomeVoucher/byBank" title="By Bank" />
          <TextButton path="/ac/IncomeVoucher/byCash" title="By Cash" />
        </div>
      </div>
