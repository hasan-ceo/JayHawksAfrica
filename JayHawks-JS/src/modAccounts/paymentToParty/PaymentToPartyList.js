...
const PaymentToPartyList = () => {
  const {
    ...
    isError,
    refetch,
  } = useGetData("paymentParty", "/paymentParty/list");

  const data = [
    {
      paymentToPartyId: "111",
      party: "AAA",
      cashOrBank: "Dhaka",
      amount: "11",
      particulars: "Dhaka",
  ...
  return (
    <div className="card w-full max-w-screen-xl">
      <TopHeader
        title="Payment To Partty"
        btn="Save"
        path={"/ac/paymentParty/add"}
      />
          ...
          data.map((item) => (
            ...
            >
              ...
              <div className="flex justify-end space-x-2">
                <DeleteButton
                  action={refetch}
                  path={`/paymentParty/delete/${item.paymentToPartyId}`}
                />
              </div>
            </div>
          ))}
...
export default PaymentToPartyList;
