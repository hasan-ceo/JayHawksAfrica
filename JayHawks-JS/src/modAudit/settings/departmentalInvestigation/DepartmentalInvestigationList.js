            ...
            >
              ...
              <div className="flex justify-end space-x-2">
                {item.status !== "Audit Close" && (
                  <>
                    <TaskButton
                      path={`/audit/departmentalInvestigation/details/${item.investigationId}`}
                    />
                    <EditButton
                      path={`/audit/departmentalInvestigation/edit/${item.investigationId}`}
                    />
                    <DeleteButton
                      action={refetch}
                      path={`/AuditDpInvestigation/delete/${item.investigationId}`}
                    />
                    <CloseButton
                      action={() => {
                        refetch();
                      }}
                      path={`/auditDpInvestigation/statusupdate/${item.investigationId}`}
                    />
                  </>
                )}
              </div>
