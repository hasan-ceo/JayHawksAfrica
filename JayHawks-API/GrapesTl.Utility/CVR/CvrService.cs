...
public class CvrService : ICvrService
{
    ...
    public async Task<CvrViewModel> Get(string cvrNumber)
    {
        try
        {
            ...
            var financialYearStartDate = !string.IsNullOrEmpty(business.Attributter.FirstOrDefault(x => x.Type == "FØRSTE_REGNSKABSPERIODE_START")?.Vaerdier?.LastOrDefault()?.Vaerdi)
